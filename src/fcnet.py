...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=32*32*3, num_classes=10,
        ...
        for item in range(1, len(hidden_dims)):
            ...
            self.params[W_keyword], self.params[b_keyword] = random_init(hidden_dims[item-1], hidden_dims[item], dtype=dtype)
    ...
    def loss(self, X, y=None):
        ...
        dr, ma = None, None
        if self.use_dropout:
            dr, ma = dropout_forward(activation, p= self.dropout_params["p"], train = self.dropout_params["train"], seed=0)
        #masks.append(mask)

        #Linear final layer
        scores = None
        if self.use_dropout:
            scores = linear_forward(dr, self.params["W"+str(self.num_layers)], self.params["b"+str(self.num_layers)])
        else:
        ...
        if self.use_dropout:
            dx, dW, db = linear_backward(dx, dr, self.params["W" + str(self.num_layers)],self.params["b" + str(self.num_layers)])
        else:
            dx, dW, db = linear_backward(dx, activation, self.params["W" + str(self.num_layers)], self.params["b" + str(self.num_layers)])
        ...
        if self.use_dropout:
            dx = dropout_backward(dx, mask=ma, p = self.dropout_params["p"], train = self.dropout_params["train"])
...
#         #######################################################################
#         # Set first hidden layer from the input dimensions and the first hidden layer dimensions
#         #self.params["W1"], self.params["b1"] = random_init(input_dim, n_out= hidden_dims[0], dtype=dtype)
#         #self.params["W2"], self.params["b2"] = random_init(hidden_dims[0], num_classes, dtype=dtype)
#         self.params["W1"] = 1e-4*np.random.randn(input_dim, hidden_dims[0])
#         self.params["b1"] = np.zeros(hidden_dims[0])
#         self.params["W2"] = 1e-4*np.random.randn(hidden_dims[0], num_classes)
...
#         activation = X
#
#         z =  linear_forward(activation, self.params["W1"], self.params["b1"])
#         activation = relu_forward(z)
#
#         #Linear final layer
#         scores = linear_forward(activation, self.params["W2"], self.params["b2"])
#         #######################################################################
#         #                            END OF YOUR CODE                         #
#         #######################################################################
#         # If y is None then we are in test mode so just return scores
...
#         grads["b2"] = db
#
#         dx = relu_backward(dx, z)
#         dx, dW, db = linear_backward(dx, X, self.params["W{}".format(1)], self.params["b{}".format(1)])
#         grads["W{}".format(1)] = dW + self.reg * self.params["W{}".format(1)]
#         grads["b{}".format(1)] = db
#         #######################################################################
#         #                            END OF YOUR CODE                         #
#         #######################################################################
