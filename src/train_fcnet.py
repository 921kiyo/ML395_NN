...
model = FullyConnectedNet(hidden_dims=[128], reg=1e-4, num_classes=10, dropout=0.1)

#model = pickle.load(open('TestModel_epoch_15.pkl','rb'))
#model = model['model']128
solver = Solver(model, data,
                update_rule='sgd',
                optim_config={'learning_rate': 0.8e-3,}, lr_decay = 0.95,
                num_epochs=50, batch_size=60,
                print_every=1000, checkpoint_name="TestModel")
